...
export class HebDateService {
  ...
  /**
   * Get Omer data for a specific day (1-49)
   */
  getOmerForDay(omerDay: number): {omer: string, zohar: string, kavana: string, hebDate: string} | null {
    if (omerDay < 1 || omerDay > 49) {
      return null;
    }
    
    const omerData = C.omerDays[omerDay - 1]; // Convert to 0-based index
    if (!omerData) {
      return null;
    }
    
    return {
      omer: omerData.getOmerString(this.settingsService.nosach),
      zohar: omerData.zohar,
      kavana: omerData.kavana,
      hebDate: omerData.fullHebDate
    };
  }
